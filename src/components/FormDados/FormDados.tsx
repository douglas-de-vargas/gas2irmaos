...
// ReactJs
import React, { useEffect, useState, useRef, ChangeEvent, FormEvent, Dispatch, SetStateAction } from 'react'
...
export default function FormDados() {
  ...
  const onSubmit = (data: IclientData) => {
    ...
    const whatsAppLink = `https://wa.me/${myphone}?text=Olá, eu gostaria de fazer um pedido!%0A*Cliente:*${' '}${
      data.name
    }%0A*Contato:*${' '}${data.phone}%0A%0A*Rua:*${' '}${data.street}${', '}${data.housenumber}%0A*Complemento:*${' '}${
      data.complement
    }%0A*Bairro:*${' '}${data.district}${' / '}${data.city}%0A*Pagamento:*${' '}${
      data.pay
  ...
  return (
    <form onSubmit={() => onSubmit}>
      <label>
        Nome:
        <input value={clientData.name} placeholder="Nome" name="name" required />
        <span style={{ fontSize: '.7rem', color: 'red' }}>Requerido!</span>
      </label>
      <label>
        Telefone de contato:
        <span style={{ fontSize: '.7rem' }}>• DDD requerido.</span>
        <input value={clientData.phone} type="number" name="phone" placeholder="(00) 12345-6789" required />
        <span style={{ fontSize: '.7rem', color: 'red' }}>Requerido!</span>
      </label>
      <label>
        Nome da rua:
        <input value={clientData.street} name="street" placeholder="Endereço" required />
        <span style={{ fontSize: '.7rem', color: 'red' }}>Requerido!</span>
      </label>
      <label>
        Número da casa:
        <input value={clientData.housenumber} type="number" name="housenumber" required placeholder="Número da casa" />
        <span style={{ fontSize: '.7rem', color: 'red' }}>Requerido!</span>
      </label>
      <label>
        Complemento:
        ...
        <input value={clientData.complement} name="complement" placeholder="Complemento" />
      </label>
      <label>
        Bairro:
        <input value={clientData.district} name="district" placeholder="Bairro" />
      </label>
      <label>
        Cidade:
      ...
      <label>
        ...
        <textarea value={clientData.additional} name="additional" placeholder="Obs" />
      </label>
